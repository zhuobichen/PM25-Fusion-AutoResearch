...
批量生成验证脚本
================
为 CodeWorkSpace/新融合方法代码/ 中缺少验证脚本的方法自动生成 *_十折标准模式.py

用法：
    python -m shared.generate_validation_scripts           # 生成所有缺失的脚本
    python -m shared.generate_validation_scripts --dry-run # 预览不生成
    python -m shared.generate_validation_scripts --method PolyRK  # 只生成指定方法
"""
...
def find_methods_needing_scripts(project_root: str) -> list:
    """找出有代码的方法，始终返回全部（不检查是否已有脚本）。"""
    code_dir = os.path.join(project_root, 'CodeWorkSpace', '新融合方法代码')
...
def generate_script(method_name: str, project_root: str, dry_run: bool = False) -> str:
    """生成单个验证脚本。"""
    script_dir = os.path.join(project_root, 'test_result', '创新方法')
    script_path = os.path.join(script_dir, f'{method_name}_十折标准模式.py')
    ...
    if dry_run:
        print(f"  [预览] {method_name}_十折标准模式.py")
        return script_path

    os.makedirs(script_dir, exist_ok=True)
    with open(script_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"  [生成] {method_name}_十折标准模式.py")

    return script_path


def main():
    parser = argparse.ArgumentParser(description='批量生成验证脚本')
    parser.add_argument('--dry-run', action='store_true', help='预览模式，不生成文件')
    parser.add_argument('--method', type=str, help='只生成指定方法的脚本')
    args = parser.parse_args()
    ...
    if args.method:
        ...
        methods = find_methods_needing_scripts(root)

    if not methods:
        print("CodeWorkSpace/新融合方法代码/ 中没有找到方法。")
        return

    print(f"{'预览' if args.dry_run else '生成'} {len(methods)} 个验证脚本:")
    for method in methods:
        generate_script(method, root, dry_run=args.dry_run)

    if not args.dry_run:
        print(f"\n完成。共生成 {len(methods)} 个脚本。")
